...
def solve_factory_machines(puzzle_input):
    ...
    for line in lines:
        ...
        queue = deque([(0, 0)])
        visited = {0}
        min_presses_for_machine = None
        
        while queue:
            curr, dist = queue.popleft()
            
            if curr == target_val:
                min_presses_for_machine = dist
                break
            
            for btn_mask in buttons:
                next_state = curr ^ btn_mask
                if next_state not in visited:
                    visited.add(next_state)
                    queue.append((next_state, dist + 1))
        
        if min_presses_for_machine is not None:
            total_presses += min_presses_for_machine
        else:
            print(f"Warning: No solution found for line: {line}")
...
print(solve_factory_machines(puzzle_input))