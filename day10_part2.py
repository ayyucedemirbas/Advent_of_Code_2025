...
def solve_min_presses_joltage(puzzle_input):
    total_min_presses = 0
    ...
    for line_idx, line in enumerate(lines):
        ...
        if not button_matches or not target_match:
            continue
            
        targets = [int(x) for x in target_match.group(1).split(',')]
        num_equations = len(targets)
        ...
        for b_str in button_matches:
            ...
            for idx in indices:
                if idx < num_equations:
                    col_vec[idx] = 1
                    limit = min(limit, targets[idx])
                    affects_something = True
            
            if not affects_something or limit == 0:
                limit = 0
                
            buttons.append(col_vec)
            var_upper_bounds.append(limit)
            
        num_vars = len(buttons)
        
        matrix = []
        for r in range(num_equations):
            row = [Fraction(buttons[c][r]) for c in range(num_vars)]
        ...
        for col in range(num_vars):
            if current_row >= num_equations:
                break
            
            pivot_row = -1
            for r in range(current_row, num_equations):
                if matrix[r][col] != 0:
                    pivot_row = r
                    break
        ...
        for r in range(len(pivot_cols)):
            ...
            for f_col in free_cols:
                val = matrix[r][f_col]
                if val != 0:
                    coeffs.append((f_col, val))
            dependencies.append((p_col, constant, coeffs))

        min_total = float('inf')
        def search(idx, current_free_vals):
            nonlocal min_total
            current_partial_sum = sum(current_free_vals.values())
            if current_partial_sum >= min_total:
                return

            if idx == len(free_cols):
                current_total = current_partial_sum
                
                for p_col, constant, coeffs in dependencies:
                    val = constant
                    for f_col, coef in coeffs:
                        val -= coef * current_free_vals[f_col]
                    
                    if val < 0 or val.denominator != 1:
                        return
                    
                    if val > var_upper_bounds[p_col]:
                        return
            ...
            f_col = free_cols[idx]
            limit = var_upper_bounds[f_col]
            
            for val in range(limit + 1):
                current_free_vals[f_col] = val
                search(idx + 1, current_free_vals)
                del current_free_vals[f_col]

        search(0, {})

        if min_total != float('inf'):
            total_min_presses += min_total
        else:
            pass
...
print(solve_min_presses_joltage(puzzle_input))